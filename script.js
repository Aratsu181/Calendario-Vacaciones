document.addEventListener("DOMContentLoaded", function () {
    ...
    vacationForm.addEventListener("submit", function (e) {
        ...
        // Save vacations with the color
        saveVacation(name, startDate, endDate, year, color);
    ...
    function saveVacation(name, startDate, endDate, year, color) {
        let vacations = JSON.parse(localStorage.getItem("vacations") || "{}");
        if (!vacations[year]) {
            vacations[year] = [];
        }
        vacations[year].push({ name, startDate, endDate, color });
    ...
    function loadVacations() {
        ...
        Object.keys(vacations).forEach(year => {
            vacations[year].forEach((vacation, index) => {
                ...
                li.innerHTML = `<span class="color-box" style="background-color:${vacation.color};"></span>${vacation.name}: ${vacation.startDate} - ${vacation.endDate} (${year})`;
        ...
        for (let month = 0; month < 12; month++) {
            ...
            for (let day = 1; day <= daysInMonth[month]; day++) {
                ...
                if (vacationColors.length > 0) {
                    cell.style.background = `linear-gradient(${vacationColors.join(', ')})`;
                }
